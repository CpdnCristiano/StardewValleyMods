    ...
    public static class ChatPatcher
    {
        ...
        public static bool Prefix(StardewArchipelagoClient __instance, LogMessage message)
        {
            try
            {
                ...
                var messagesToIgnoreField = typeof(StardewArchipelagoClient)
                    .GetField("_messagesToIgnore", BindingFlags.Instance | BindingFlags.NonPublic);
                var messagesToIgnore = (List<string>?)messagesToIgnoreField?.GetValue(__instance);

                if (messagesToIgnore != null && (messagesToIgnore.Any(x => x.Contains(fullMessage) || fullMessage.Contains(x)) ||
                    messagesToIgnore.Any(x => x.Contains(stardewFullMessage) || fullMessage.Contains(x))))
                {
                    var loggerField = typeof(StardewArchipelagoClient)
                        .GetField("Logger", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
                    if (loggerField == null)
                    {
                        loggerField = typeof(StardewArchipelagoClient).BaseType?
                            .GetField("Logger", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
                    }
                    var logger = (KaitoKid.Utilities.Interfaces.ILogger?)loggerField?.GetValue(__instance);
                    logger?.LogDebug($"Ignoring Chat Message: {fullMessage}");
                    return false;
                }

                var activeLoggerField = typeof(StardewArchipelagoClient)
                    .GetField("Logger", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
                if (activeLoggerField == null)
                {
                    activeLoggerField = typeof(StardewArchipelagoClient).BaseType?
                        .GetField("Logger", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
                }
                var activeLogger = (KaitoKid.Utilities.Interfaces.ILogger?)activeLoggerField?.GetValue(__instance);
                activeLogger?.LogInfo(fullMessage);

                stardewFullMessage = stardewFullMessage.AnonymizePlayerNames(__instance.GetSession().Players);
                if (stardewFullMessage.Trim().StartsWith("[Hint]:", StringComparison.OrdinalIgnoreCase))
                {
                    stardewFullMessage = TranslationHelper.TranslateHintMessage(stardewFullMessage.Trim());
                }
                ...
                switch (message)
                {
                    ...
                    case ItemSendLogMessage itemSendLogMessage:
                        {
                            if (StardewArchipelago.ModEntry.Instance.Config.DisplayItemsInChat == ChatItemsFilter.None)
                            {
                                return false;
                            }

                            if (StardewArchipelago.ModEntry.Instance.Config.DisplayItemsInChat == ChatItemsFilter.RelatedToMe && !itemSendLogMessage.IsRelatedToActivePlayer)
                            {
                                return false;
                            }
            ...
            {
                ModEntry.Instance.Monitor.Log($"Error in OnMessageReceived Prefix, falling back to original: {ex}", LogLevel.Error);
                return true;
            }
        }
