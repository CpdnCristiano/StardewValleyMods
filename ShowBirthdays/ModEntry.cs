...
namespace ShowBirthdays
{
	class ModEntry : Mod
	{
		...
		private void OnRenderedActiveMenu(object? sender, RenderedActiveMenuEventArgs e)
		{
			...
			for (int i = 0; i < listOfDays.Count; i++)
			{
                ...
                if (billboard.calendarDayData.TryGetValue(clickableTextureComponent.myID, out var value))
                {
                    ...
                    List<NPC>? sharedBirthdays = bdHelper.GetNpcs(Game1.season, listOfDays[i]);
                    if (sharedBirthdays is { Count: > 1 })
                    {
                        DrawSharedBirthdaySprites(e.SpriteBatch, clickableTextureComponent, value.TextureSourceRect, sharedBirthdays);
                    }
                    else if (days[listOfDays[i] - 1].texture != null && value.Texture != null)
                    {
        ...
        private void DrawSharedBirthdaySprites(
            SpriteBatch spriteBatch,
            ClickableTextureComponent day,
        ...
        )
        {
            ...
            int availableWidth = day.bounds.Width - horizontalPadding * 2;
            int availableHeight = day.bounds.Height - verticalPadding * 2;
            float scaleX = (availableWidth - spacing * (npcs.Count - 1)) / (float)(sourceRect.Width * npcs.Count);
            float scaleY = availableHeight / (float)sourceRect.Height;
            float scale = Math.Min(4f, MathF.Max(1.4f, MathF.Min(scaleX, scaleY)));

            float spriteWidth = sourceRect.Width * scale;
            float spriteHeight = sourceRect.Height * scale;
            float totalWidth = spriteWidth * npcs.Count + spacing * (npcs.Count - 1);
            float startX = day.bounds.X + (day.bounds.Width - totalWidth) / 2f;
            float startY = day.bounds.Bottom - verticalPadding - spriteHeight;

            for (int i = 0; i < npcs.Count; i++)
            {
                Texture2D texture = TryGetNpcTexture(npcs[i]);
                Vector2 drawPos = new Vector2(startX + i * (spriteWidth + spacing), startY);
                spriteBatch.Draw(texture, drawPos, sourceRect, Color.White, 0f, Vector2.Zero, scale, SpriteEffects.None, 1f);
            }
        }

        private Texture2D TryGetNpcTexture(NPC npc)
        {
            try
            {
                return Helper.GameContent.Load<Texture2D>($"Characters\\{npc.getTextureName()}");
            }
            catch
            {
